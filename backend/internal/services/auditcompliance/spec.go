...
func Spec() platform.ServiceSpec {
	...
	return platform.ServiceSpec{
		...
		Tables:      []string{"audit_logs", "security_findings", "security_reports", "event_ingestion_offsets", "project_report_members", "group_report_members", "outbox", "inbox"},
		Events:      []string{"AuditEvent"},
		Routes: []platform.RouteSpec{
			route(http.MethodPost, "/api/v1/audit/events", "audit_events", "event_ingest"),
			route(http.MethodGet, "/api/v1/audit/logs", "audit_logs", "list"),
