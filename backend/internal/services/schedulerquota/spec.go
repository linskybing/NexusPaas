...
func Spec() platform.ServiceSpec {
	...
	return platform.ServiceSpec{
		...
		Events:          []string{"PlanChanged", "QueueChanged", "QuotaReserved", "QuotaCommitted", "QuotaReleased", "SubmitAdmissionReviewed", "SecretAccessRejected", "QueueDepthChanged", "JobPreempted", "PriorityClassSyncCompleted", "NetworkProfileChanged", "PlacementProfileChanged", "AcceleratorProfileChanged"},
		Routes: []platform.RouteSpec{
			...
			route(http.MethodGet, "/api/v1/accelerator-profiles/{id}", "accelerator_profiles", "get", id("id"), admin()),
			route(http.MethodPut, "/api/v1/accelerator-profiles/{id}", "accelerator_profiles", "update", id("id"), admin()),
			route(http.MethodDelete, "/api/v1/accelerator-profiles/{id}", "accelerator_profiles", "delete", id("id"), admin()),
			...
			route(http.MethodGet, "/api/v1/network-profiles/{id}", "network_profiles", "get", id("id"), admin()),
			route(http.MethodPut, "/api/v1/network-profiles/{id}", "network_profiles", "update", id("id"), admin()),
			route(http.MethodDelete, "/api/v1/network-profiles/{id}", "network_profiles", "delete", id("id"), admin()),
			...
			route(http.MethodGet, "/api/v1/placement-profiles/{id}", "placement_profiles", "get", id("id"), admin()),
			route(http.MethodPut, "/api/v1/placement-profiles/{id}", "placement_profiles", "update", id("id"), admin()),
			route(http.MethodDelete, "/api/v1/placement-profiles/{id}", "placement_profiles", "delete", id("id"), admin()),
