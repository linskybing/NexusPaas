...
func TestProductionBetaLiveRehearsalHarnessIsGuarded(t *testing.T) {
	...
	requireContains(t, scriptPath, script, "production-beta render contains forbidden SERVICE_NAME=all")
	requireContains(t, scriptPath, script, "rendered_service_name_for_unit")
	requireContains(t, scriptPath, script, "validate_render_service_names")
	requireContains(t, scriptPath, script, "production-beta render ConfigMap ${unit}-config SERVICE_NAME=${service_name:-<missing>}, want ${unit}")
