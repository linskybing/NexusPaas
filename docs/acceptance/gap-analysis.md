...
for `POST /internal/storage/projects/{project_id}/data-plane-plan`: an
in-memory storage-service app rejects a wrong `X-Service-Key`, returns the
default `local-nvme-scratch` scratch and `cephfs-rwx-authority` checkpoint
profiles, ignores forged request source details, returns storage-owned stage-in
PVC references, and emits `DataPlanePlanBuilt`. These local/static/in-memory
checks do not prove live permission enforcement, live kind/Kubernetes mount or
