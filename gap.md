...
| Storage DataPlane cache-hit runtime in kind | evidence id `2026-06-28-storage-data-plane-cache-hit-kind-runtime-e2e`: `backend/internal/e2e/storage_data_plane_cache_hit_kind_runtime_e2e_test.go` | Done for env-gated kind cache-hit runtime evidence only: storage-service built a cache-hit DataPlanePlan, workload-service dispatched a Pod using a pre-created scratch PVC, the Pod reached `Succeeded`, injected checkpoint env matched the plan, a marker was written to and read back from the scratch PVC, and no stage target PVC was materialized. No stage-in byte copy, CSI/local NVMe/CephFS/Longhorn runtime, scratch PVC auto-provisioning, checkpoint flush, performance, multi-node behavior, storage GA, Full GA, or V1 external production launch readiness claim |
